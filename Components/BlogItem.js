...
export default function BlogItem({ data }) {
    ...
    return (
        <div className="blog_item">
            <Link href={{ pathname: '/blog', query: { id: data.id } }}>
                <a href="" className="w-100 h-100">
                    {
                        data.file ?
                            <img className="mb-3" src={"https://api-admin.shafa.care/api/v3/auth/publicFile/" + data.file.path} alt="blog" /> :
                            <img className="mb-3" src={blogimg1} alt="blog-img" />
                    }
